...
public abstract class AbstractPouchHP extends ItemHP {
    ...
    public int getMaxSlots(ItemStack pouch) {
        int gluttonyLevel = ModEnchantsHP.enableGluttonousEnchant ? EnchantmentHelper.getEnchantmentLevel(ModEnchantsHP.GLUTTONOUS, pouch) : ConfigHandlerHP.HUNGRY_POUCH_ENCHANTS.GLUTTONOUS.pouchFallbackSize;
        ...
        if(player.isSneaking()) {
            ...
            if(!(tile != null && tile.hasCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, trace.sideHit))) {
                if (PouchHandler.isEnabled(stack)) {
                    ...
                    player.sendStatusMessage(new TextComponentTranslation(StringHelper.getTranslationKey("disabled", "chat")), true);
                } else {
                    PouchHandler.setEnabled(stack, true);
                    player.sendStatusMessage(new TextComponentTranslation(StringHelper.getTranslationKey("enabled", "chat")), true);
                }
            }
        } else {
            if (!player.isCreative() && ConfigHandlerHP.GENERAL_SETTINGS.enableBiting && PouchHandler.isEnabled(stack) && !player.world.getDifficulty().equals(EnumDifficulty.PEACEFUL) && !(stack.getItem() instanceof ItemPouchSkeletal)) {
                ...
                player.sendStatusMessage(new TextComponentTranslation(StringHelper.getTranslationKey("biting", "chat")), true);
    ...
    protected void addDefaultTooltip(ItemStack stack, List<String> tooltip) {
        ...
        tooltip.add(I18n.format(StringHelper.getTranslationKey("slotinfo", "tooltip"), PouchHandler.getUsedSlots(stack), PouchHandler.getMaxSlots(stack)));
    }
    ...
    protected void addNoKeyTooltip(ItemStack stack, List<String> tooltip) {
        if(ConfigHandlerHP.GENERAL_SETTINGS.enableUsageTooltips) {
            tooltip.add(I18n.format(StringHelper.getTranslationKey("info", "tooltip")));
            tooltip.add(I18n.format(StringHelper.getTranslationKey("usage", "tooltip")));
        }
    }
    ...
    protected void addCtrlTooltip(ItemStack stack, List<String> tooltip) {
        ...
        tooltip.add(I18n.format(StringHelper.getTranslationKey("transferinfo", "tooltip")));
        tooltip.add(I18n.format(StringHelper.getTranslationKey("shuffleinfo", "tooltip")));
    }
